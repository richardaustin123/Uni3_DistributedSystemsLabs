...
void flipRow(int row, vector<vector<int>>* m1, vector<vector<int>>* m2);
void flipColumn(int col, vector<vector<int>>* m2, vector<vector<int>>* mResult);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            start = chrono::high_resolution_clock::now();       
            {
                ...
                for (int row = 0; row < matrixSize; ++row) {
                    pool.enqueue([row, &m1, &m2]() {
                        flipRow(row, &m1, &m2);
                    });
                }

                for (int col = 0; col < matrixSize; ++col) {
                    pool.enqueue([col, &m2, &mResult]() {
                        flipColumn(col, &m2, &mResult);
                    });
                }
            }
...
// flipRow( row, m1, mResult)
// flip(original rows, original matrix, flipped matrix)
void flipRow(int row, vector<vector<int>>* m1, vector<vector<int>>* m2) {
...
// flipColumn( col, m1, mResult)
// flip(flipped columns, matrix with flipped rows, final flipped matrix)
void flipColumn(int col, vector<vector<int>>* m2, vector<vector<int>>* mResult) {
