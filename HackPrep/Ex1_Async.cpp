...
int main() {
    ...
    if (outputFile.is_open()) { 
        ...
        for (int i = 0; i < 10; i++) {
            ...
            {
                // Now flip the matrix
                future<void> flipRowsFuture;
                flipRowsFuture = async(flipRows, &m1, &m2);
                flipRowsFuture.get(); // Wait for flipRows to finish
                
                future<void> flipColumnsFuture; 
                flipColumnsFuture = async(flipColumns, &m2, &mResult);
                flipColumnsFuture.get(); // Wait for flipColumns to finish
            }
